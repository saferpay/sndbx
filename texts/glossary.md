...
<table class="table table-striped table-hover">
  ...
  <tbody>
    ...
    <tr>
      <td class="text-center"><strong>OrderId</strong></td>
      <td><p>This Id is your identifier, for a given order. It has to be created by your system and then be submitted through the API inside the <strong>Payment.OrderId</strong> parameter. While not mandatory (Note, that this may be manddatory certain 3rd party processors), we highly recommend using it. This Id will be passed through, all the way up to the processor. It will show up inside the Saferpay Backoffice as reference number, which enables you, to search for a transaction with said orderId. It will be also show up on your reconciliation files you get from your processor and it will show up on the card holders bank statement. Note, that this depends on the card holders bank. Some do not support it, even though OrderId is set! Also, should you use PayerNote (See PayerNote), this will be used instead of the OrderId, to show up on the card holders bank statement, if supported by his bank. The OrderId however will still be used for your reconciliation files.</p></td>
